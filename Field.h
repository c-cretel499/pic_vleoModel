...
template <typename T> // typename just gives us a way to identify the type 
struct vec3 {
...
//vec3 - vector functions, pulled from DSMC code
template<typename T>
...
template<typename T>
vec3<T> unit(const vec3<T>& a) {
	double m = mag(a);
	return { a(0) / m, a(1) / m, a(2) / m };
}
template<typename T>
double mag(const vec3<T>& a) {
